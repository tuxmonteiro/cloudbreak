...
public class EC2ClientActions extends EC2Client {
    ...
    public List<String> getMasterVolumeIds(TestContext testContext, SdxTestDto testDto, SdxClient sdxClient) {
        AmazonEC2 ec2Client = buildEC2Client();
        List<String> instanceIds = getInstanceIds(testDto, sdxClient);
        ...
        volumeIds.forEach(volumeId -> Log.log(LOGGER, format(" Found volume ID: %s ", volumeId)));
        return volumeIds;
    }
    ...
    public SdxTestDto compareVolumeIdsAfterRepair(SdxTestDto sdxTestDto, List<String> actualVolumeIds, List<String> expectedVolumeIds) {
        ...
        expectedVolumeIds.forEach(volumeId -> Log.log(LOGGER, format(" Expected volume ID: %s ", volumeId)));

        if (!actualVolumeIds.equals(expectedVolumeIds)) {
            LOGGER.error("Host Group: {} does not have the desired volume IDs!", HostGroupType.MASTER.getName());
            actualVolumeIds.forEach(volumeid -> Log.log(LOGGER, format(" Actual volume ID: %s ", volumeid)));
            expectedVolumeIds.forEach(volumeId -> Log.log(LOGGER, format(" Desired volume ID: %s ", volumeId)));
            throw new TestFailException("Host Group: " +  HostGroupType.MASTER.getName() + " does not have the desired volume IDs!");
        }
        return sdxTestDto;
    }
    ...
    private List<String> getInstanceIds(SdxTestDto testDto, SdxClient sdxClient) {
        ...
        InstanceGroupV4Response instanceGroupV4Response = sdxClient.getSdxClient().sdxEndpoint().getDetail(testDto.getName(), entries)
                .getStackV4Response().getInstanceGroups().stream().filter(instanceGroup -> instanceGroup.getName().equals(HostGroupType.MASTER.getName()))
                .findFirst()
                .orElse(null);
